...
(define section "4.1.3. Procedure calls") 
; 
...
; 
; 4.1.4. Procedures
(define section "4.1.4. Procedures") 
; 
...
(test section '((lambda (x) (+ x x)) 4)        8)

(define reverse-subtract
  (lambda (x y) (- y x)))
; (reverse-subtract 7 10)         =  3
(test section '(reverse-subtract 7 10)         3)

(define add4
  (let ((x 4))
    (lambda (y) (+ x y))))
; (add4 6)                        =  10
(test section '(add4 6)                       10)
...
; ((lambda (x y . z) z)
; 3 4 5 6)                       =  (5 6)
(test section '((lambda x x) 3 4 5 6)         '(3 4 5 6))
(test section '((lambda (x y . z) z) 3 4 5 6) '(5 6))
; 
...
; 
; 4.1.5. Conditionals
(define section "4.1.5. Conditionals") 
; 
...
(test section '(if (> 3 2) 'yes 'no)          'yes)
(test section '(if (> 2 3) 'yes 'no)          'no)
(test section '(if (> 3 2) (- 3 2) (+ 3 2))    1)

; 
; 
; 4.1.6. Assignments
(define section "4.1.6. Assignments") 
; 
...
(define section "4.2. Derived expression types") 
; 
...
; 4.2.1. Conditionals
(define section "4.2.1. Conditionals") 
; 
...
; (cond ((> 3 2) 'greater)
; ((< 3 2) 'less))          =  greater
(test section '(cond ((> 3 2) 'greater) ((< 3 2) 'less)) 'greater)
...
; (else #f))                =  2
(test section '(cond ((> 3 3) 'greater) ((< 3 3) 'less) (else 'equal)) 'equal)
...
; (and 1 2 'c '(f g))             =  (f g)
; (and)                           =  #t
(test section '(and (= 2 2) (> 2 1))          #t)
(test section '(and (= 2 2) (< 2 1))          #f)
(test section '(and 1 2 'c '(f g))            '(f g))
(test section '(and)                          #t)
...
; (or (memq 'b '(a b c))
; (/ 3 0))                   =  (b c)
(test section '(or (= 2 2) (> 2 1))     #t)
(test section '(or (= 2 2) (< 2 1))     #t)
(test section '(or #f #f #f)            #f)
(test section '(or (memq 'b '(a b c)) (/ 3 0)) '(b c))

; 
; 
; 4.2.2. Binding constructs
(define section "4.2.2. Binding constructs") 
; 
...
; (z (+ x y)))
; (* z x)))                   =  35
(test section '(let ((x 2) (y 3)) (* x y)) 6)
(test section '(let ((x 2) (y 3)) (let ((x 7) (z (+ x y))) (* z x))) 35)
...
; 
; 4.2.3. Sequencing
(define section "4.2.3. Sequencing") 
; 
...
; (begin (set! x 5)
; (+ x 1))               =  6
(test section '(begin (set! x 5) (+ x 1))           6)
...
; 
; 4.2.4. Iteration
(define section "4.2.4. Iteration") 
; 
...
(define section "4.2.5. Delayed evaluation") 
; 
...
(define section "4.2.6. Quasiquotation") 
; 
...
;(test section '(quasiquote (list (unquote (+ 1 2)) 4)) '(list 3 4))

(test section ''(quasiquote (list (unquote (+ 1 2)) 4)) '(quasiquote (list (unquote (+ 1 2)) 4)))
...
(define section "4.3.1. Binding constructs for syntactic keywords") 
; 
...
(define section "4.3.2. Pattern language") 
; 
...
; 
; 5.      Program structure
(define section "5. Program structure") 
; 5.1. Programs
(define section "5.1. Programs") 
; 
...
; 
; 5.2. Definitions
(define section "5.2. Definitions") 
; 
...
(define section "5.2.1. Top level definitions") 
; 
...
(test section '(add3 3)        6)
(define first car)
(test section '(first '(1 2)) 1)
...
(define section "5.2.2. Internal definitions") 
; 
...
(define section "5.3. Syntax definitions") 
; 
...
(define section "6. Standard procedures") 
...
(define section "6.1. Equivalence predicates") 
; 
...
;(test section '(string=? (symbol->string obj1) (symbol->string obj2)) #t)
...
; (test section '(string=? (symbol->string obj1) (symbol->string obj2)) #f)
...
(test section '(eqv? 'a 'a)                          #t)
(test section '(eqv? 'a 'b)                          #f)
(test section '(eqv? 2 2)                            #t)
(test section '(eqv? '() '())                        #t)
(test section '(eqv? 100000000 100000000)            #t)
(test section '(eqv? (cons 1 2) (cons 1 2))          #f)
(test section '(eqv? (lambda () 1) (lambda () 2))    #f)
(test section '(eqv? #f 'nil)                        #f)
(test section '(let ((p (lambda (x) x))) (eqv? p p)) #t)
...
; =  #f
(test section '(let ((g (gen-counter))) (eqv? g g)) #t)
(test section '(eqv? (gen-counter) (gen-counter)) #f)
...
; (let ((g (gen-loser)))
; (eqv? g g))                  =  #t
(test section '(let ((g (gen-loser))) (eqv? g g))    #t)
...
(test section '(let ((x '(a))) (eqv? x x))        #t)
; 
...
; (eq? p p))                     =  #t
; 
(test section '(eq? 'a 'a)                        #t)
(test section '(eq? (list 'a) (list 'a))           #f)
(test section '(eq? '() '())                       #t)
(test section '(eq? car car)                       #t)
(test section '(let ((x '(a))) (eq? x x))                       #t)
(test section '(let ((x '#())) (eq? x x))                       #t)
(test section '(let ((p (lambda (x) x))) (eq? p p))                       #t)
; 
; 
...
(test section '(equal? 'a 'a)                  #t)
(test section '(equal? '(a) '(a))              #t)
(test section '(equal? '(a (b) c) '(a (b) c))  #t)
(test section '(equal? "abc" "abc")            #t)
(test section '(equal? 2 2)                    #t)
(test section '(equal? (make-vector 5 'a) (make-vector 5 'a)) #t)
; 
; 
; 6.2. Numbers
(define section "6.2. Numbers") 
; 
...
(define section "6.2.1. Numerical types") 
; 
...
; 
; 6.2.2. Exactness
(define section "6.2.2. Exactness") 
; 
...
(define section "6.2.3. Implementation restrictions") 
; 
...
(define section "6.2.4. Syntax of numerical constants") 
; 
...
(define section "6.2.5. Numerical operations") 
; 
...
(test section '(string->number "100")          100)
...
(define section "6.3. Other data types") 
; 
...
; (not (list))                     =  #f
; (not 'nil)                       =  #f
(test section '(not #t)                        #f)
(test section '(not 3)                         #f)
(test section '(not (list 3))                  #f)
(test section '(not #f)                        #t)
(test section '(not '())                       #f)
(test section '(not (list))                    #f)
(test section '(not 'nil)                      #f)
...
(test section '(boolean? #f)                #t)
(test section '(boolean? 0)                 #f)
(test section '(boolean? '())               #f)

; 
; 
; 6.3.2. Pairs and lists
(define section "6.3.2. Pairs and lists") 
; 
...
(define x (list 'a 'b 'c))
(define y x)
(test section 'y  '(a b c))
(test section '(list? y) #t)
(set-cdr! x 4);                 =  unspecified
(test section 'x  '(a . 4))
(test section '(eqv? x y) #t)
(test section 'y '(a . 4))
(test section '(list? y) #f)
(set-cdr! x x) ;                =  unspecified
(test section '(list? x) #f)
...
; (pair? '())                    =  #f
; (pair? '#(a b))                =  #f
(test section '(pair? '(a . b))                 #t)
(test section '(pair? '(a b c))                 #t)
(test section '(pair? '())                      #f)
(test section '(pair? '#(a b))                  #f)
; 
...
(test section '(cons 'a '())                    '(a))
(test section '(cons '(a) '(b c d))             '((a) b c d))
(test section '(cons "a" '(b c))                '("a" b c))
(test section '(cons 'a 3)                      '(a . 3))
(test section '(cons '(a b) 'c)                 '((a b) . c))
...
(test section '(car '(a b c))                    'a)
(test section '(car '((a) b c d))                '(a))
(test section '(car '(1 . 2))                    1)
...
(test section '(cdr '((a) b c d))                '(b c d))
(test section '(cdr '(1 . 2))                    2)
; 
; 
...
(test section '(list? '(a b c))        #t)
(test section '(list? '())             #t)
(test section '(list? '(a . b))        #f)
(test section '(let ((x (list 'a)))
	 (set-cdr! x x)
	 (list? x))
      #f)
; 
...
; (list 'a (+ 3 4) 'c)            =  (a 7 c)
; (list)                          =  ()
(test section '(list 'a (+ 3 4) 'c)              '(a 7 c))
(test section '(list)                            '())
; 
...
; (length '())                    =  0
; 
(test section '(length '(a b c))                 3)
(test section '(length '(a (b) (c d e)))         3)
(test section '(length '())                      0)
...
(test section '(append '(x) '(y))                '(x y))
(test section '(append '(a) '(b c d))            '(a b c d))
(test section '(append '(a (b)) '((c)))          '(a (b) (c)))
; 
...
; (reverse '(a (b c) d (e (f))))
; =  ((e (f)) d (b c) a)
(test section '(reverse '(a b c))                '(c b a))
(test section '(reverse '(a (b c) d (e (f))))   '((e (f)) d (b c) a))
; 
; 
...
(test section '(memq 'a '(a b c))                  '(a b c))
(test section '(memq 'b '(a b c))                  '(b c))
(test section '(memq 'a '(b c d))                  #f)
(test section '(memq (list 'a) '(b (a) c))         #f)
(test section '(member (list 'a) '(b (a) c))       '((a) c))
(test section '(memv 101 '(100 101 102))           '(101 102))
; 
; 
...
(define e '((a 1) (b 2) (c 3)))
(test section '(assq 'a e)                            '(a 1))
(test section '(assq 'b e)                            '(b 2))
(test section '(assq 'd e)                            #f)
(test section '(assq (list 'a) '(((a)) ((b)) ((c))))  #f)
(test section '(assoc (list 'a) '(((a)) ((b)) ((c)))) '((a)))
(test section '(assv 5 '((2 3) (5 7) (11 13)))        '(5 7))
...
(test section '(symbol? 'foo)                      #t)
(test section '(symbol? (car '(a b)))              #t)
(test section '(symbol? "bar")                     #f)
(test section '(symbol? 'nil)                      #t)
(test section '(symbol? '())                       #f)
(test section '(symbol? #f)                        #f)
...
(test section '(symbol->string 'flying-fish)               "flying-fish")
; (test section '(symbol->string 'Martin)                    "martin")
(test section '(symbol->string (string->symbol "Malvina")) "Malvina")
; 
; 
...
; =  #t
; 
; (test section '(eq? 'mISSISSIppi 'mississippi)   #t)
; (string->symbol "mISSISSIppi")
; =  the symbol with name "mISSISSIppi"
; (test section '(eq? 'bitBlt (string->symbol "bitBlt"))   #f)
(test section '(eq? 'JollyWog
	    (string->symbol (symbol->string 'JollyWog)))
      #t)
; 
; 
; 6.3.4. Characters
(define section "6.3.4. Characters") 
; 
...
; (vector 'a 'b 'c)           =  #(a b c)
(test section '(vector 'a 'b 'c)             '#(a b c))
; 
...
; 5)
; =  8
(test section '(vector-ref '#(1 1 2 3 5 8 13 21) 5)   8)
; (vector-ref '#(1 1 2 3 5 8 13 21)
...
(test section '(let ((vec (vector 0 '(2 2 2 2) "Anna")))
	 (vector-set! vec 1 '("Sue" "Sue"))
	 vec)
      '#(0 ("Sue" "Sue") "Anna"))
; 
...
(test section '(vector->list '#(dah dah didah))   '(dah dah didah))
(test section '(list->vector '(dididit dah))      '#(dididit dah))
; 
...
(define section "6.4. Control features") 
; 
...
(test section '(procedure? car)                   #t)
(test section '(procedure? 'car)                  #f)
(test section '(procedure? (lambda (x) (* x x)))   #t)
(test section '(procedure? '(lambda (x) (* x x)))   #f)
(test section '(call-with-current-continuation procedure?)   #t)
; 
; 
; 
; 32 Revised5 Scheme
...
; =  (b e h)
(test section '(map cadr '((a b) (d e) (g h)))   '(b e h))
; 
...
(test section '(let ((v (make-vector 5)))
	 (for-each (lambda (i)
		     (vector-set! v i (* i i)))
		   '(0 1 2 3 4))
	 v)
...
(test section '(force (delay (+ 1 2)))
      3)
(test section '(let ((p (delay (+ 1 2))))
...
(test section '(call-with-current-continuation
	(lambda (exit)
	  (for-each (lambda (x)
		      (if (negative? x)
			  (exit x)))
...
; (test section '(list-length '(1 2 3 4)) 4)
; (test section '(list-length '(a b . c)) #f)
; 
; Rationale:
...
(test section '(eval '(* 7 3) (scheme-report-environment 5))  21)
; 
...
(test section '(let ((f (eval '(lambda (f x) (f x x))
		      (null-environment 5))))
	 (f + 10))
      20)
; 
...
(define section "6.6. Input and output") 
; 
; 6.6.1. Ports
(define section "6.6.1. Ports") 
; 
...
(define section "6.6.4. System interface") 
; 
