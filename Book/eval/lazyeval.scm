...
(define (list-of-args pars args env)
  (if (no-operands? args)
      '()
      (let ((first-par (first-par pars))
	    (rest-pars (rest-pars pars))
	    (first-operand (first-operand args))
	    (rest-operands (rest-operands args)))
	(cond ((par-lazy? first-par)
	       (cons (thunk-make first-operand env)
		     (list-of-args rest-pars rest-operands env)))
	      ((par-lazy-memo? first-par)
	       (cons (memo-thunk-make first-operand env)
		     (list-of-args rest-pars rest-operands env)))
	      (else
	       (cons (actual-value first-operand env)
		     (list-of-args rest-pars rest-operands env)))))))
...
(define (par-lazy-memo? par)
  (if (list? par)
      (eq? (cadr par) 'lazy-memo)
      false))

(define (par-name lazy-par)
  (car lazy-par))

(define (first-par pars) (car pars))
(define (rest-pars pars) (cdr pars))

;-----------------------------------
; Thunks


(define (thunk-make exp env)
  (list 'thunk exp env))

(define (memo-thunk-make exp env)
